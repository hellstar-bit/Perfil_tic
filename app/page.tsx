...
/* ══════════════════════════════════════════
   MOBILE
══════════════════════════════════════════ */
function LandingMobile({ perfiles, proyectos }: { perfiles: number; proyectos: number }) {
  return (
    <div className="min-h-dvh font-sans text-ink-900 md:hidden" style={{ background: "#0E0E0E" }}>
      ...
      <section className="mx-5 mb-10 rounded-[14px] border border-ink-200 bg-ink-50 p-4 grid grid-cols-3 divide-x divide-ink-200">
        {[
          ...
          [String(proyectos), "proyectos publicados"],
      ...
      <section className="mx-5 mb-10 rounded-[16px] p-6" style={{ background: "#161616", border: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="section-label mb-3">03 · IA integrada</div>
        <TypedHeading lines={["5 herramientas de IA incluidas"]} className="text-[22px] font-bold mb-3 leading-tight" />
        <div className="flex flex-col gap-2 mb-5">
          {[
            ["Frase de presentación", "Genera tu resumen profesional con un clic."],
            ["Mejora descripciones", "Optimiza tus experiencias y formación."],
            ["Coach de perfil", "Detecta qué le falta a tu CV y cómo mejorarlo."],
            ["Lee certificados", "Sube un PDF y extrae los datos automáticamente."],
            ["Simulador de entrevista", "Practica preguntas reales antes de postularte."],
          ].map(([n, d]) => (
            <div key={n} className="flex items-start gap-2 text-[12px]">
              <span className="text-neon mt-0.5 shrink-0"><CheckIcon /></span>
              <span><span className="font-semibold text-ink-900">{n}</span><span className="text-ink-500"> — {d}</span></span>
            </div>
          ))}
        </div>
...
/* ══════════════════════════════════════════
   DESKTOP
══════════════════════════════════════════ */
function LandingDesktop({ perfiles, proyectos }: { perfiles: number; proyectos: number }) {
  return (
    <div className="font-sans text-ink-900 hidden md:block" style={{ background: "#0E0E0E" }}>
      ...
      <header className="flex items-center justify-between px-12 pt-5 pb-4 sticky top-0 z-10 border-b border-ink-200"
        ...
        <nav className="flex items-center gap-8 text-sm text-ink-600">
          ...
          <a href="#ia" className="hover:text-ink-900 transition-colors">IA integrada</a>
      ...
      {/* IA */}
      <section id="ia" className="px-12 py-24 max-w-[1280px] mx-auto">
        <div className="grid grid-cols-[1fr_400px] gap-16 items-center">
          <div>
            <div className="section-label mb-4">03 · IA integrada</div>
            <TypedHeading lines={["5 herramientas de IA", "incluidas sin costo extra."]} className="text-[44px] font-bold tracking-tight leading-[1.1] mb-5" />
            <p className="text-[16px] text-ink-500 leading-relaxed mb-8 max-w-lg">
              Desde generar tu frase de presentación hasta simular una entrevista técnica — todo dentro del editor, sin salir de la plataforma.
            </p>
            <Link href="/registro" className="h-12 px-7 rounded-[8px] bg-neon text-noir font-bold text-[15px] inline-flex items-center gap-2 hover:brightness-90 transition-all">
              Crea tu perfil gratis <Arrow />
            </Link>
          </div>
          ...
          <div className="card p-7 space-y-4">
            {[
              ["Frase de presentación", "Genera tu resumen profesional con un clic."],
              ["Mejora descripciones", "Optimiza experiencias y formación con IA."],
              ["Coach de perfil", "Detecta qué le falta a tu CV y cómo mejorarlo."],
              ["Lee certificados", "Sube un PDF y extrae los datos automáticamente."],
              ["Simulador de entrevista", "Practica preguntas reales antes de postularte."],
            ].map(([n, d]) => (
              <div key={n} className="flex items-start gap-3">
                <span className="text-neon shrink-0 mt-0.5"><CheckIcon /></span>
                <div>
                  <div className="font-semibold text-[13px] text-ink-900">{n}</div>
                  <div className="text-[12px] text-ink-500 mt-0.5">{d}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-[1fr_auto_auto] gap-16 items-start">
            ...
            <div>
              ...
              <div className="space-y-2 text-[13px] text-ink-600">
                ...
                <a href="#ia" className="block hover:text-ink-900">IA integrada</a>
...
export default async function Home() {
  ...
  return (
    <>
      <LandingMobile perfiles={perfiles} proyectos={proyectos} />
      <LandingDesktop perfiles={perfiles} proyectos={proyectos} />
    </>
  );
}
